...
class TestLedger(unittest.TestCase):
    ...
    def test_instantation_and_insert(self):
        ...
        for d in datelist[:10]:
            _ = l.log_transactions(d, [1.0, 1.0], [1.0, 1.0])
        """
        ...
        """
        tax_implications = l.log_transactions(str(datelist[10]), [-5, -4], [1.0, 1.0])
        print(f"Tax implications: {tax_implications}")
        self.assertEqual(tax_implications["assets"][0]["long_term_shares"], 3)
        self.assertEqual(tax_implications["assets"][0]["short_term_shares"], 2)
        self.assertEqual(
            tax_implications["assets"][0]["short_term_shares"]
            + tax_implications["assets"][0]["long_term_shares"],
            5,
        )
        self.assertTrue(1 in tax_implications["assets"])
        """
        Since we have now exhausted our early purchases in terms of long term, we should get entirely short term shares here

        """
        trans2 = [-4, -4]
        tax_implications = l.log_transactions(str(datelist[11]), trans2, [1.0, 1.0])
        print(tax_implications)
        for i, v in enumerate(trans2):
            self.assertEqual(tax_implications["assets"][i]["short_term_shares"], -v)
    ...
    def test_count_holdings(self):
        ...
        tax_imps = l.log_transactions(datelist[-2], [-2, -2], [1.0, 1.0])
