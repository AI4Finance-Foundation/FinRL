...
class TestStocktradingEnvCashpenalty(unittest.TestCase):
    ...
    def test_zero_step(self):
        # Prove that zero actions results in zero stock buys, and no price
        # changes
        init_amt = 1e6
    ...
    def test_shares_increment(self):
        # Prove that we can only buy/sell multiplies of shares based on
        # shares_increment parameter
        aapl_first_close = self.df[self.df["tic"] == "AAPL"].head(1)[
            "close"].values[0]
        init_amt = 1e6
    ...
    def test_patient(self):
        # Prove that we just not buying any new assets if running out of cash
