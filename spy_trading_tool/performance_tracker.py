...
class PerformanceTracker:
    ...
    def calculate_metrics(self) -> Dict:
        ...
        if len(returns) > 1 and np.std(returns) > 0:
            sharpe = (np.mean(returns) - self.risk_free_rate/252) / np.std(returns) * np.sqrt(252)
        else:
            sharpe = 0
    ...
    def plot_equity_curve(self, save_path: Optional[str] = None, show: bool = True):
        ...
        # Plot equity curve
        ax.plot(self.timestamps, self.equity_curve, linewidth=2, label='Portfolio Value')
        ax.axhline(y=self.initial_capital, color='gray', linestyle='--', label='Initial Capital')

        # Fill area
        ax.fill_between(self.timestamps, self.equity_curve, self.initial_capital,
    ...
    def plot_returns_distribution(self, save_path: Optional[str] = None, show: bool = True):
        ...
        ax.set_title('SPY Trading - Returns Distribution', fontsize=14, fontweight='bold')
        ax.legend(loc='best')
