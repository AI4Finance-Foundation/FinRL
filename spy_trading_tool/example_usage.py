...
def example_training():
    ...
    tool = SPYTradingTool(initial_capital=10000, save_dir="./example_models")
    ...
    tool.train_initial_model(
        training_days=7, timesteps=1000  # Use 7 days for quick demo  # Reduced for demo
    )
...
def example_timeframe_optimization():
    ...
    tool = SPYTradingTool(
        timeframes=["1m", "5m", "15m"], initial_capital=10000  # Reduced set for demo
    )
...
def example_continuous_trading():
    ...
    try:
        tool.run_continuous(
            update_interval=10, max_updates=5  # 10 seconds for demo  # Only 5 updates
        )
...
def example_performance_tracking():
    ...
    # Create tracker
    tracker = PerformanceTracker(initial_capital=10000, log_file="./example_trades.log")
