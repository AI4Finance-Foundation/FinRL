...
def main():
    ...
    ax1.plot(x, norm(bah_vals),   label="Buy & Hold", linewidth=1.0, color="green", linestyle="--")
    ...
    ax2 = axes[1]
    ax2.fill_between(x, drawdown_pct(assets_ppo), 0, alpha=0.4,
                     color="steelblue",  label="PPO DD")
    ax2.fill_between(x, drawdown_pct(assets_sac), 0, alpha=0.4,
                     color="darkorange", label="SAC DD")
    ax2.fill_between(x, drawdown_pct(bah_vals),   0, alpha=0.3,
                     color="green",      label="Buy & Hold DD")
