    ...
    def get_rl_prediction(self, df):
        ...
        try:
            ...
            signal = {
                ...
                "signal": (
                    "BUY" if action[0] > 0.2 else "SELL" if action[0] < -0.2 else "HOLD"
                ),
        ...
        while True:
            try:
                ...
                if rl_signal:
                    self.logger.info("\nRL Model Prediction:")
                    self.logger.info(
                        f"Signal: {rl_signal['signal']} (Strength: {rl_signal['strength']:.2f})"
                    )
                ...
                tech_sentiment = sum(
                    [
                        (
                            1
                            if s["signal"] == "BUY"
                            else -1 if s["signal"] == "SELL" else 0
                        )
                        for s in tech_signals.values()
                    ]
                ) / len(tech_signals)
                ...
                overall_signal = (
                    ...
                    if final_sentiment > 0.5
                        ...
                        if final_sentiment > 0.2
                            ...
                            "STRONG SELL"
                            if final_sentiment < -0.5
                            else "SELL" if final_sentiment < -0.2 else "HOLD"
                        )
                    )
                )
