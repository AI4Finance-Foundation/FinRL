...
try:
    ...
    print("Warning: YahooDownloader not found for get_baseline. Ensure it's in the correct path or install FinRL correctly.")
    ...
    class YahooDownloader:
        def __init__(self, start_date, end_date, ticker_list):
            ...
            print("Dummy YahooDownloader initialized because actual could not be imported.")
        def fetch_data(self):
            print(f"Dummy YahooDownloader: Would fetch {self.ticker_list} from {self.start_date} to {self.end_date}")
...
DEFAULT_TRADE_END_DATE = pd.Timestamp.today().strftime('%Y-%m-%d') # Use current date as a sensible default
...
def get_daily_return(df, value_col_name="account_value"):
    ...
    if 'date' not in df.columns:
        if df.index.name == 'date' and isinstance(df.index, pd.DatetimeIndex):
            ...
            raise KeyError(f"DataFrame is missing the 'date' column for get_daily_return. Columns: {df.columns}")
    ...
    if df.index.tz is not None: # If already localized, pyfolio might handle it or prefer naive
...
def backtest_plot(
    ...
    if 'date' not in df.columns:
        raise KeyError(f"Input DataFrame to backtest_plot is missing 'date' column. Columns: {df.columns}")
    if value_col_name not in df.columns:
        raise KeyError(f"Input DataFrame to backtest_plot is missing value column '{value_col_name}'. Columns: {df.columns}")
    ...
    test_returns = get_daily_return(df.copy(), value_col_name=value_col_name) # Pass a copy to get_daily_return
    ...
    if baseline_ticker is not None:
        ...
        if not baseline_df_raw.empty and 'date' in baseline_df_raw.columns and 'close' in baseline_df_raw.columns:
            # get_daily_return for baseline also expects 'date' and 'close' as columns
            baseline_returns_series = get_daily_return(baseline_df_raw.copy(), value_col_name="close")
            baseline_returns = baseline_returns_series.to_frame(name=baseline_ticker) # Convert to DataFrame for pyfolio
            # Align baseline returns with test_returns
            baseline_returns = baseline_returns.reindex(test_returns.index).fillna(0.0)
        else:
            print(f"Warning: Baseline ticker {baseline_ticker} data is empty or malformed. Plotting without baseline.")
    ...
    try:
        ...
        if baseline_returns is not None:
            pyfolio_baseline_returns = baseline_returns[baseline_ticker].copy() # Get Series
            if pyfolio_baseline_returns.index.tz is not None:
                 pyfolio_baseline_returns.index = pyfolio_baseline_returns.index.tz_localize(None)
        ...
        df_for_simple_plot = df.copy().set_index('date') # df already has 'date' as datetime column
        ...
        if baseline_returns is not None and not baseline_df_raw.empty:
            ...
            print("Simple plot does not include baseline value line, only agent's account value.")
            pass
...
def backtest_stats(account_value, value_col_name="account_value"):
    ...
    if not isinstance(account_value, pd.DataFrame):
        raise TypeError("Input 'account_value' to backtest_stats must be a pandas DataFrame.")
    if 'date' not in account_value.columns:
        raise KeyError(f"Input DataFrame to backtest_stats is missing 'date' column. Columns: {account_value.columns}")
    if value_col_name not in account_value.columns:
        raise KeyError(f"Input DataFrame to backtest_stats is missing value column '{value_col_name}'. Columns: {account_value.columns}")
