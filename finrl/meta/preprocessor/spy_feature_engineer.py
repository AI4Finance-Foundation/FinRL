...
class SPYFeatureEngineer(FeatureEngineer):
    ...
    def __init__(
        ...
        self.options_processor = OptionsProcessor(ticker='SPY', risk_free_rate=risk_free_rate)

    def preprocess_spy_data(self, df: pd.DataFrame,
                           include_options: bool = True) -> pd.DataFrame:
        """Main method to add all SPY-specific features.
    ...
    def add_advanced_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for indicator_name, indicator_key in advanced_indicators.items():
            ...
            if not indicator_df.empty:
                df = df.merge(indicator_df[['tic', 'date', indicator_key]], on=['tic', 'date'], how='left')
    ...
    def add_volume_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in unique_ticker:
            ...
            # Volume ratio
            ticker_df['volume_ratio'] = ticker_df['volume'] / (ticker_df['volume_sma_20'] + 1e-10)
    ...
    def add_regime_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in unique_ticker:
            ...
            volatility_regime = pd.cut(
                atr_percent,
                bins=[0, atr_percent.quantile(0.33), atr_percent.quantile(0.67), float('inf')],
                labels=[0, 1, 2]
            ).astype(float)
    ...
    def add_options_greeks(self, data: pd.DataFrame,
        ...
        try:
            ...
            # Calculate Greeks
            options_with_greeks = self.options_processor.calculate_greeks(options_df, current_price)
            ...
            call_volume = options_with_greeks[options_with_greeks['optionType'] == 'call']['volume'].sum()
            put_volume = options_with_greeks[options_with_greeks['optionType'] == 'put']['volume'].sum()
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.use_advanced_indicators:
            features.extend([
                'rsi_14', 'rsi_7', 'macdh', 'atr', 'adx',
                'kdjk', 'kdjd', 'wr_14', 'roc_5', 'roc_10'
            ])
        ...
        if self.use_options_greeks:
            features.extend([
                'call_delta', 'call_gamma', 'call_theta', 'call_vega', 'call_iv', 'call_strike',
                'put_delta', 'put_gamma', 'put_theta', 'put_vega', 'put_iv', 'put_strike',
                'put_call_ratio', 'atm_iv'
            ])
