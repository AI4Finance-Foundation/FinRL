...
class YahooDownloader:
    ...
    def fetch_data(self, proxy=None) -> pd.DataFrame:
        ...
        for tic_i in self.ticker_list: # Renamed 'tic' to 'tic_i' for clarity in loop
            temp_df = yf.download(
                tic_i, start=self.start_date, end=self.end_date, proxy=proxy
            ...
            if temp_df.empty:
                print(f"No data fetched for {tic_i} from {self.start_date} to {self.end_date}")
                num_failures += 1
                continue
        ...
        if num_failures == len(self.ticker_list):
            ...
            raise ValueError(f"No data could be fetched for any ticker in the list: {self.ticker_list}")
        ...
        if data_df.empty: # if still empty after loop (all failed)
             raise ValueError(f"DataFrame is empty after attempting to fetch all tickers.")

        data_df = data_df.reset_index() # Reset index to turn 'Date' (or 'Datetime') into a column
        ...
        if 'adj close' in data_df.columns:
            ...
            if 'adjcp' not in data_df.columns: # If not created from 'adj close'
                data_df['adjcp'] = data_df['close'] # Make adjcp same as close (which is adjusted)
        else:
        ...
        try:
            ...
            current_cols = ['date', 'open', 'high', 'low', 'close', 'adjcp', 'volume', 'tic']
            if not all(col in data_df.columns for col in current_cols):
                missing_cols = [col for col in current_cols if col not in data_df.columns]
                # This should not happen if logic above is correct
                raise ValueError(f"DataFrame is missing essential columns: {missing_cols}. Current columns: {data_df.columns.tolist()}")
