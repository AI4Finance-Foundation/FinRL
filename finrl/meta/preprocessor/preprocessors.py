...
try:
    ...
    class ConfigFallback:
        INDICATORS = ["macd", "rsi_30", "cci_30", "adx"] # Match your paper trading script
...
class FeatureEngineer:
    ...
    def clean_data(self, data):
        ...
        if 'date' in df.columns:
            ...
            print("Warning: 'date' column not found in clean_data. Index not changed by factorize.")
        ...
        try:
            ...
            print(f"Error during pivoting/cleaning in clean_data: {e}. Returning original df sorted.")
    ...
    def add_technical_indicator(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"Error converting 'date' column to datetime: {e}. Ensure 'date' is parsable.")
            for indicator_name_spec in self.tech_indicator_list:
                if indicator_name_spec not in df.columns: df[indicator_name_spec] = np.nan
            return df.sort_values(by=["date", "tic"]).reset_index(drop=True)
        
        ohlcv_cols = ['open', 'high', 'low', 'close', 'volume']
        for col in ohlcv_cols:
            if col not in df.columns:
                print(f"Warning: Essential column '{col}' missing. Cannot calculate TA indicators fully.")
                for indicator_name_spec in self.tech_indicator_list:
                     if indicator_name_spec not in df.columns: df[indicator_name_spec] = np.nan
                return df.sort_values(by=["date", "tic"]).reset_index(drop=True)
        ...
        if df.empty:
            print("DataFrame became empty after dropping NaNs in OHLCV. Cannot add indicators.")
            # Add NaN columns to original `data` if it was passed in, not the modified `df`
            for indicator_name_spec in self.tech_indicator_list:
                 if indicator_name_spec not in data.columns: data[indicator_name_spec] = np.nan
        ...
        for tic_name in df.tic.unique():
            ...
            if 'macd' in self.tech_indicator_list:
                try:
                    ...
                    if macd_results is not None and not macd_results.empty and 'MACD_12_26_9' in macd_results.columns:
                        tic_df['macd'] = macd_results['MACD_12_26_9']
                    else: tic_df['macd'] = np.nan
                except Exception as e: tic_df['macd'] = np.nan; print(f"Err macd {tic_name}: {e}")
            ...
            if 'rsi_30' in self.tech_indicator_list:
                try:
                    ...
                    else: tic_df['rsi_30'] = np.nan
                except Exception as e: tic_df['rsi_30'] = np.nan; print(f"Err rsi_30 {tic_name}: {e}")
            
            if 'cci_30' in self.tech_indicator_list:
                try:
                    cci_results = tic_df.ta.cci(length=30, high='high', low='low', close='close', append=False)
                    if cci_results is not None and not cci_results.empty:
                        tic_df['cci_30'] = cci_results
                    else: tic_df['cci_30'] = np.nan
                except Exception as e: tic_df['cci_30'] = np.nan; print(f"Err cci_30 {tic_name}: {e}")
            ...
            if 'adx_30' in self.tech_indicator_list: # Check for 'adx_30'
                try:
                    ...
                    adx_results = tic_df.ta.adx(length=30, lensig=30, high='high', low='low', close='close', append=False)
                    if adx_results is not None and 'ADX_30' in adx_results.columns: # pandas-ta names it ADX_LENSIG
                        tic_df['adx_30'] = adx_results['ADX_30'] # Create column 'adx_30'
                    else: 
                        print(f"Warning: pandas-ta did not produce 'ADX_30' column for {tic_name}. 'adx_30' will be NaN.")
                        tic_df['adx_30'] = np.nan
                except Exception as e: 
                    tic_df['adx_30'] = np.nan
                    print(f"Err adx_30 for {tic_name}: {e}")
            # --- END MODIFIED ADX PART ---
            elif 'adx' in self.tech_indicator_list: # Fallback if user just asks for 'adx' (default 14 period)
                try:
                    adx_results = tic_df.ta.adx(length=14, high='high', low='low', close='close', append=False)
                    if adx_results is not None and 'ADX_14' in adx_results.columns:
                        tic_df['adx'] = adx_results['ADX_14']
                    else: tic_df['adx'] = np.nan
                except Exception as e: tic_df['adx'] = np.nan; print(f"Err adx for {tic_name}: {e}")
    
            tic_df.reset_index(inplace=True)
            all_indicator_data_list.append(tic_df)
        
        if not all_indicator_data_list:
            print("No data processed by pandas-ta. Returning original df with NaN indicators.")
            for indicator_name_spec in self.tech_indicator_list:
                if indicator_name_spec not in df.columns: df[indicator_name_spec] = np.nan
            return df.sort_values(by=["date", "tic"]).reset_index(drop=True)
        ...
        cols_to_merge_from_ta = ['date', 'tic'] + [ind for ind in self.tech_indicator_list if ind in processed_with_ta_df.columns]
        
        mergeable_indicator_df = processed_with_ta_df[cols_to_merge_from_ta].drop_duplicates(subset=['date', 'tic'], keep='last')
        
        if mergeable_indicator_df.empty:
            print("Mergeable indicator df is empty after selection/deduplication. Returning original with NaN indicators.")
            for indicator_name_spec in self.tech_indicator_list:
                if indicator_name_spec not in data.columns: data[indicator_name_spec] = np.nan # Add to original `data`
            return data.sort_values(by=["date", "tic"]).reset_index(drop=True) # Return original `data`
        ...
        output_df = output_df.merge(mergeable_indicator_df, on=['date', 'tic'], how='left')
    ...
    def add_user_defined_feature(self, data):
        ...
        df["daily_return"] = df.groupby('tic')['close'].transform(lambda x: x.pct_change(1))
        return df
    ...
    def add_vix(self, data):
        ...
        if df_vix.empty or 'close' not in df_vix.columns or 'date' not in df_vix.columns:
    ...
    def add_turbulence(self, data):
        ...
        try:
            ...
            print(f"Error calculating or merging turbulence: {e}. Turbulence not added.")
            if 'turbulence' not in df.columns: df['turbulence'] = np.nan
        return df
    ...
    def calculate_turbulence(self, data):
        ...
        if len(unique_date) < start:
            print("Not enough unique dates for turbulence calculation (need at least 252).")
        ...
        for i in range(start, len(unique_date)):
            ...
            common_tickers = hist_price.columns.intersection(current_price_slice.columns)
            hist_price_aligned = hist_price[common_tickers].dropna(axis=1, how='all').dropna(axis=0, how='any')
            current_price_aligned = current_price_slice[common_tickers]

            if hist_price_aligned.shape[0] < 2 or hist_price_aligned.shape[1] < 1 or current_price_aligned.isnull().values.any():
            ...
            current_return_diff = (current_price_aligned.iloc[0] - np.mean(hist_price_aligned, axis=0)).values.reshape(1, -1)
        ...
        try:
            ...
            print(f"Error creating turbulence DataFrame: {e_turb_df}. Turbulence information could not be added.")
