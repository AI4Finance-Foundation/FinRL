...
class StockTradingEnv(gym.Env): # Inherit from gym.Env
    ...
    metadata = {"render_modes": ["human"], "render_fps": 1} # render_modes and render_fps for gymnasium
    ...
    def __init__(
        ...
        self.initial_stocks = num_stock_shares # More common name for initial holdings, used later
        ...
        if self.day >= len(self.unique_dates):
             raise ValueError(f"Initial day {self.day} is out of bounds for unique dates {len(self.unique_dates)}.")
        
        current_date_for_init = self.unique_dates[self.day]
        self.data = self.df[self.df.date == current_date_for_init].reset_index(drop=True)
        if self.data.empty:
            raise ValueError(f"No data found for the initial date: {current_date_for_init}. Check DataFrame.")
        # --- End FIX ---

        self.terminal = False
        self.make_plots = make_plots
        self.print_verbosity = print_verbosity
        self.turbulence_threshold = turbulence_threshold # if not None, turbulence logic is active
        self.risk_indicator_col = risk_indicator_col # Name of the turbulence column in df
        ...
        # Initialize state
        self.state = self._initiate_state() # This will now use the correctly shaped self.data
        ...
        if self.data.empty or not isinstance(self.data.close, pd.Series) or self.data.close.empty:
            raise ValueError("self.data or self.data.close is not correctly populated for asset_memory calculation.")
        
        initial_portfolio_value = np.sum(
            np.array(self.initial_stocks) * self.data.close.values # self.data.close is now a Series
        )
    ...
    def _sell_stock(self, index, action):
        def _do_sell_normal():
            if self.state[index + 1] > 0:  # Price of stock i
                sell_num_shares = min(abs(action), self.state[index + self.stock_dim + 1]) # Current shares of stock i
                sell_amount = (
                    self.state[index + 1]
                    * sell_num_shares
    ...
    def step(self, actions): # actions from agent, shape (self.action_space_dim,)
        ...
        if self.terminal:
            ...
            if len(self.date_memory) == len(df_total_value):
                 ...
                 df_total_value["date"] = pd.to_datetime(self.unique_dates[:len(df_total_value)])
            ...
            if self.episode % self.print_verbosity == 0:
                ...
                print(f"day: {self.day}, episode: {self.episode}") # self.day is final day index
                print(f"begin_total_asset: {self.asset_memory[0]:0.2f}")
                print(f"end_total_asset: {final_portfolio_value:0.2f}")
                print(f"total_accumulated_rewards_memory: {np.sum(self.rewards_memory):0.2f}")
                print(f"total_cost: {self.cost:0.2f}")
                print(f"total_trades: {self.trades}")
                if sharpe != 0.0 : print(f"Sharpe: {sharpe:0.3f}")
                print("=================================")
            ...
            if (self.model_name != "") and (self.mode != ""):
                ...
                if self.rewards_memory:
                    df_rewards_mem = pd.DataFrame(self.rewards_memory, columns=["reward"])
                    # Ensure date_memory for rewards is appropriate (usually one less than asset_memory)
                    if len(self.date_memory) > 1 and len(df_rewards_mem) == len(self.date_memory) -1:
                        df_rewards_mem["date"] = self.date_memory[1:] # Rewards correspond to t+1 state
                    elif len(df_rewards_mem) == len(self.unique_dates) -1 :
                         df_rewards_mem["date"] = self.unique_dates[1:len(df_rewards_mem)+1]
            ...
            if self.turbulence_threshold is not None and self.risk_indicator_col in self.data.columns:
            ...
            self.data = self.df[self.df.date == current_date_for_step].reset_index(drop=True)
            if self.data.empty:
                raise ValueError(f"No data found for date {current_date_for_step} in step. Day index {self.day}")
    ...
    def reset(
        ...
        *, # Gymnasium requires keyword-only arguments after this
        ...
        super().reset(seed=seed) # Call super for gym.Env compatibility if needed for seeding
        
        self.day = 0 # Reset day to the start
        current_date_for_reset = self.unique_dates[self.day]
        self.data = self.df[self.df.date == current_date_for_reset].reset_index(drop=True)
        if self.data.empty:
            raise ValueError(f"No data found for initial date {current_date_for_reset} in reset.")
        ...
        if self.initial: # self.initial is a flag passed during __init__
            ...
            initial_portfolio_value = np.sum(
                 np.array(self.initial_stocks) * self.data.close.values # self.data is for day 0
            )
            self.asset_memory = [self.initial_amount + initial_portfolio_value]
        else:
            ...
            previous_shares = np.array(self.previous_state[1 + self.stock_dim : 1 + 2 * self.stock_dim])
    ...
    def render(self, mode="human"): # mode is for gymnasium compatibility
        ...
        if mode == 'human':
            ...
            print(f"Portfolio Value: {self.asset_memory[-1] if self.asset_memory else 'N/A'}")
    ...
    def _initiate_state(self):
        ...
        if self.data.empty or not isinstance(self.data.close, pd.Series) or len(self.data.close) != self.stock_dim:
            raise ValueError(f"self.data.close is not a Series of expected length {self.stock_dim} for state initiation. "
        ...
        if self.initial:
            ...
            if self.stock_dim > 1: # Multiple stocks
                state = (
                    ...
                    + sum(
                        (
                            self.data[tech].values.tolist() # Tech indicators for all stocks
                ...
                state = (
                    ...
                    + sum(([self.data[tech].iloc[0]] for tech in self.tech_indicator_list), [])
                )
        else:
            ...
            if not self.previous_state or len(self.previous_state) != self.state_space_dim:
                 raise ValueError("previous_state is invalid or has incorrect dimension for re-initialization.")

            if self.stock_dim > 1:
                state = (
                    [self.previous_state[0]] # Previous cash
                    + self.data.close.values.tolist() # Current prices
                    + self.previous_state[1 + self.stock_dim : 1 + 2 * self.stock_dim] # Previous shares
                    + sum(
                        (
        ...
        if len(state) != self.state_space_dim:
            raise ValueError(f"Constructed state length {len(state)} does not match expected state_space dimension {self.state_space_dim}.")
    ...
    def _update_state(self):
        ...
        if self.data.empty or not isinstance(self.data.close, pd.Series) or len(self.data.close) != self.stock_dim:
            raise ValueError(f"self.data.close is not a Series of expected length {self.stock_dim} for state update.")

        if self.stock_dim > 1:
            state = (
                [self.state[0]] # Current cash
                + self.data.close.values.tolist() # New prices for all stocks
                + list(self.state[1 + self.stock_dim : 1 + 2 * self.stock_dim]) # Current shares owned
                + sum(
                    (
                        self.data[tech].values.tolist() # New tech indicators for all stocks
                        for tech in self.tech_indicator_list
                    ),
                    [],
                )
            ...
            state = (
                ...
                + list(self.state[1 + self.stock_dim : 1 + 2 * self.stock_dim]) # Current shares
                + sum(([self.data[tech].iloc[0]] for tech in self.tech_indicator_list), []) # New tech indicators
            )
        
        if len(state) != self.state_space_dim:
            raise ValueError(f"Updated state length {len(state)} does not match expected state_space dimension {self.state_space_dim}.")
        return np.array(state, dtype=np.float32)
    ...
    def _get_date(self):
        ...
        if not self.data.empty:
            return self.data.date.iloc[0] # Get date from the first row (should be same for all)
        elif self.unique_dates: # Fallback if self.data somehow became empty but we have dates
            return self.unique_dates[self.day if self.day < len(self.unique_dates) else -1]
        return "N/A" # Should not happen

    def save_state_memory(self): # This method seems specific to a fixed number of assets (Bitcoin, Gold)
        ...
        if len(self.df.tic.unique()) > 1:
            date_list = self.date_memory[:-1] # Exclude current day's date as state is for t
            if not date_list: return pd.DataFrame() # No memory to save

            df_date = pd.DataFrame(date_list, columns=["date"])

            state_list = self.state_memory
            if not state_list: return pd.DataFrame()

            # Dynamic column naming based on stock_dim and tech_indicator_list
            price_cols = [f"{self.data.tic.iloc[i]}_price" for i in range(self.stock_dim)]
            ...
            tech_cols = [f"tech_{j}_{i}" for j in range(self.stock_dim) for i in range(num_tech_features_per_stock)]
            ...
            if state_list and len(state_list[0]) != len(state_df_columns):
                 print(f"Warning: Mismatch in state memory columns. Expected {len(state_df_columns)}, got {len(state_list[0]) if state_list else 'N/A'}")
                 # Fallback to generic numbered columns if mismatch
                 state_df_columns = [f"state_{i}" for i in range(len(state_list[0]))]


            df_states = pd.DataFrame(state_list, columns=state_df_columns[:len(state_list[0])]) # Slice columns if mismatch
            ...
            df_states = pd.DataFrame({"date": date_list, "states": state_list}) # Original simpler version
        return df_states

    def save_asset_memory(self):
        date_list = self.date_memory
        asset_list = self.asset_memory
        if not date_list or not asset_list: return pd.DataFrame()
    ...
    def save_action_memory(self):
        if not self.actions_memory: return pd.DataFrame()

        date_list = self.date_memory[:-1] # Actions correspond to state at t, leading to state t+1
        if not date_list: return pd.DataFrame()
        ...
        if not action_list_aligned: return pd.DataFrame()
        ...
        if self.stock_dim > 1:
            ...
            if not self.df.empty and 'tic' in self.df.columns:
                tic_names = self.df.tic.unique()[:self.stock_dim] # Get unique tickers up to stock_dim
                if len(tic_names) == df_actions.shape[1]:
                     df_actions.columns = tic_names
                else: # Fallback if tic names don't match action dimensions
                     df_actions.columns = [f"action_stock_{i}" for i in range(df_actions.shape[1])]
            else: # Fallback
                df_actions.columns = [f"action_stock_{i}" for i in range(df_actions.shape[1])]

            df_actions.index = df_date.date
        else: # Single stock
            df_actions = pd.DataFrame({"date": date_list_aligned, "actions": [a[0] for a in action_list_aligned]}) # Assuming actions is list of lists/arrays
        return df_actions
