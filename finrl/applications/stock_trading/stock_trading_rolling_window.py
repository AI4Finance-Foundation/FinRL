...
def main():
    ...
    (
        ...
        trade_starts,
        trade_ends,
    ) = calc_train_trade_starts_ends_during_trade(
        init_train_dates, init_trade_dates, num_subsets_during_trade
    )
    ...
    for i in range(len(train_starts)):
        ...
        result_a2c, actions_a2c = (
            DRLAgent.DRL_prediction(model=trained_model, environment=e_trade_gym)
            if if_using_a2c
            else None,
            None,
        )
        ...
        if store_actions:
            actions_a2c.to_csv(
                "actions_a2c" + str(i) + ".csv"
            ) if if_using_a2c else None
            actions_ddpg.to_csv(
                "actions_ddpg" + str(i) + ".csv"
            ) if if_using_ddpg else None
            actions_td3.to_csv(
                "actions_td3" + str(i) + ".csv"
            ) if if_using_td3 else None
            actions_ppo.to_csv(
                "actions_ppo" + str(i) + ".csv"
            ) if if_using_ppo else None
            actions_sac.to_csv(
                "actions_sac" + str(i) + ".csv"
            ) if if_using_sac else None

        # baseline stats
        print("==============Get Baseline Stats===========")
        df_dji_ = get_baseline(ticker="^DJI", start=trade_start, end=trade_end)
        ...
        df_dji["account_value"] = (
            df_dji_["close"]
            / df_dji_["close"].tolist()[0]
            * env_kwargs["initial_amount"]
        )
        ...
        for col in result.columns:
            if (
                col != "date"
                and result[col].tolist()[0] != env_kwargs["initial_amount"]
            ):
                result[col] = (
                    result[col] / result[col].tolist()[0] * env_kwargs["initial_amount"]
                )
